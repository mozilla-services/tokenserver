...
class TestLDAPNode(TestCase, RegPatcher):
    ...
    def setUp(self):
        ...
        self.ini = os.path.join(os.path.dirname(__file__), 'test_ldapnode.ini')
        settings = {}
    ...
    def test_return_right_node(self):
        # if we ask for a particular email/service/version association which
    ...
    def test_return_none_when_unknown(self):
        # if there is no record of the wanted user, the backend should return
        # a hash
        ...
        node, username = self.backend.get_node("alexis@mozilla.com", "sync")
